...
| ~~B9~~ | ~~3D terrain view~~ | **Done.** DEM relief from measured LiDAR, pasture colouring draped over it, and livestock markers shaped and coloured by what the animal is - a species file declares that now rather than the views guessing from a class id that happens to begin "sheep_". The map also names the stock under it, because a mark that does not say whether it is sheep or cattle withholds the one thing a person at the gate could tell you |
| ~~B9b~~ | ~~Real elevation for at least one farm~~ | **Done for LURDF.** `data/scenarios/lincoln-lurdf` runs on Canterbury - Selwyn LiDAR 1m DEM (2023), fetched by `scripts/nz-elevation-snapshot.py` from LINZ's open elevation on S3, which needs no key - the Data Service refused one, answering `Invalid API key scope` to both the export and layers APIs, and offering no WCS. Reading it needs `tiff[lerc]`; the manifest asks for it. The other two example farms are still on generated ground, and no farm yet has a real **boundary** - that is open item 15, and a different problem: the cadastre has no owner field |
