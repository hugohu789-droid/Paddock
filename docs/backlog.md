...
| B9b | Real elevation for at least one farm | **The tile is fetched.** `scripts/nz-elevation-snapshot.py` takes it from LINZ's open elevation on S3, which needs no key at all - the Data Service refused one, answering `Invalid API key scope` to both the export and layers APIs, and offering no WCS. What is left is reading it: the tile is LERC-compressed and libtiff must carry that codec, and a scenario needs a `[terrain]` kind that names a file the way `[boundary]` already names a GeoPackage. Note the earlier claim here that `layer-104931` covers Lincoln was wrong - that collection's bounding box does and not one of its 290 tiles does |
