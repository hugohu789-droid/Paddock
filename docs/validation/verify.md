...
| E64 | **The next suspect for the seasonal shape, on evidence rather than a hunch: `optimum_temperature_c`, which is a PLACEHOLDER.** After E63 the model's spring is still 19 points light and spring is where the trial puts more than half its year. With a base of 4.4 C and an optimum of 20.0, the triangular response gives Canterbury spring (11.9 C mean) 48% and summer (16.9 C) 80% - a ratio of 1.67 in summer's favour, before radiation adds another 1.24. Those two numbers are the whole of the model's summer advantage, and one of them is marked PLACEHOLDER in `sward.toml` and was never read out of anything. **It must not be tuned until it is sourced** - moving it would close the gap for the wrong reason and would be E40's mistake a second time. What is wanted is a published cardinal temperature set for perennial ryegrass, and then whatever shape is left over after the sourced value is in | M5 |
