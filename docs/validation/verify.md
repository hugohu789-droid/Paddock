...
| E20 | **CLOSED. M4's drought acceptance passes.** The driest of Lincoln's ten recorded years takes the farm below the cover it is held to, the mob goes short, and the farmer sells; the wettest year does not. It took three things, each recorded here as it was found: a ewe charged for the pregnancy she carries and the milk she makes, her lambs grazing (E22), and the pasture calibrated (E21). The dry year now costs $74,905 in bought feed against the wet year's $34,525, and the farm closes it $2,000 down against $38,000 up - **the drought shows in the books as well as in the grass**, which is what a farmer would recognise | M4 |
...
| E26 | **A third to a half of this model's standing dry matter is dead**, where a grazed New Zealand pasture runs 10 to 20%. Cover reached 5,087 kg DM/ha in a wet February with 2,208 of it dead - a thatch no animal would touch. Management reads cover rather than green, and that is correct rather than an oversight: the 1,600 kg DM/ha floor a policy states is a rising plate meter figure and a plate meter reads dead material too, so measuring green against a threshold set in cover silently raises the bar by a third. Switching it was tried and reverted - it had the farmer buying feed all year. **The defect is upstream in the sward**: senescence at a flat 2% a day of green with decomposition at 2% a day of dead, and no trampling or treading loss at all, lets dead accumulate in a way a grazed pasture does not. `green_kg_dm_per_ha` is now reported beside cover so the gap is visible rather than implied | M2 |
| E27 | **The seasonality is not a Canterbury seasonality.** Cover bottoms out in September and peaks in February; a real Canterbury dryland farm bottoms out in the summer dry. What the model produces is a spring feed deficit - lambing and lactation demand against cold-season growth - rather than a summer drought, and the dry year's low point is in the wrong season for the right arithmetic. The water stress coefficient is not the cause: it falls to 0.26 through November and December, which is severe. Growth is simply large enough in summer that even a quarter of it outruns the stock. Worth investigating against the monthly shape in `data/calibration/dairynz-pasture-growth.csv`, which the model has never been compared to month by month | M2 |
...
| E28 | **This farm sells store lambs where its own cost survey finishes them.** The whole lamb crop leaves at weaning at about 18 kg, so the drafting rule - which wants 38 kg liveweight - never fires. That is a store system, and `data/economics/canterbury-sheep.toml` is Beef + Lamb's Class 6, "S.I. Finishing Breeding". A farm that finished its lambs would earn more per head and eat considerably more grass doing it, so the costs and the enterprise are describing two different farms. Not a defect in either, but the two should be made to agree before a whole-farm margin is quoted | M5 |
...
| E31 | **What the leaching figure counts, and the three things it does not.** Nitrate leaving the 60 cm root zone from urine patches, by a mixing model - the share of the pool that goes is the share of the water that goes. Excluded, and stated in every report that prints the number: leaching from between the patches, which OVERSEER puts at under 15% of a grazed block's loss, so this reads low by about that much; runoff, direct deposition into water and artificial drainage; and any attenuation between the root zone and a waterway. Measured: 8.4 kg N/ha in the driest of ten years, 8.7 in an ordinary one, 27.3 in the wettest, against a national sheep-and-beef figure near 17 and a Selwyn Waihora trigger of 15. The urine patch loading (1000 kg N/ha, the top of OVERSEER's stated range) and patch uptake (300, PLACEHOLDER) are the two parameters the total is most sensitive to | M4 |
