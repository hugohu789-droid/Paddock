...
**So the acute single-dose figures must not be used as a cumulative threshold.** The dose to GGT step has to be calibrated empirically against the field spore-count thresholds instead, and labelled as an empirical index rather than presented as toxin mass balance. The two ends stay mechanistic and cited; the middle is fitted, and says so. Also unverified: a figure of about 10^5 spores/g dry pasture as outbreak level, seen only in secondary sources, and a 25% three-week herd production drop attributed to AgResearch trials on a commercial page with no citation |
